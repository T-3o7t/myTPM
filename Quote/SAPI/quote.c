...
int main(int argc, char *argv[]){
    ...
    /* attestationData(quote.size, 通常32バイトよりずっと大きい)をSHA-256で
     * ハッシュしてから、その32バイトのダイジェストをPCR16へextendする。
     * 以前はattestationDataをそのままdigest.sha256(32バイト固定)へmemcpy
     * しており、quote.sizeが32を超えるとスタックバッファオーバーフローに
     * なっていた。 */
    ...
    TPML_DIGEST_VALUES value = {0};
    value.count = 1;
    value.digests[0].hashAlg = TPM2_ALG_SHA256;
    memcpy(value.digests[0].digest.sha256, hash_quote.buffer, hash_quote.size);
